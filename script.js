...
function beginBattle(){
	bottomRow.innerHTML = "มาเริ่มกันเลยไหม!";
...
function enemyAttack(){
	...
	bottomRow.innerHTML += "<br>คุณรับความเสียหาย " + dmg + " ดาเมจ! เหลือพลังชีวิต " + plyrHP + " HP.";
	var playerHPBarWidth = (plyrHP/100)*300;
	playerHP.style.width = playerHPBarWidth + "px";
	if (plyrHP == 0){
		bottomRow.innerHTML += "<br>คุณแพ้แล้ว!.";
		bottomRow.innerHTML += "<br>โอ้ โห้! คุณแพ้!<br><button onclick='restartGame()'class='buttonStyle'>เริ่มเกมใหม่!</button>";
...
function attack(){
	...
	bottomRow.innerHTML = "คุณตีพวกเขา, สร้างความเสียหาย " + dmg + " ดาเมจ! เหลือพลังชีวิต " + enmyHP + " HP.";
	var enemyHPBarWidth = (enmyHP/100)*300;
	enemyHP.style.width = enemyHPBarWidth + "px";
	
	if (enmyHP == 0){
		bottomRow.innerHTML += "<br>คุณทำสำเร็จ!";
		bottomRow.innerHTML += "<br>ยินดีด้วย! คุณชนะ!<br><button onclick='restartGame()'class='buttonStyle'>เริ่มเกมใหม่!</button>";
		playerMoves.style.visibility = "hidden";
		battleMusic.pause();
		battleMusic.currentTime = 0;
		victoryMusic.play();
...
function specialAttack(){
	...
	if (enmyHP == 0){
		bottomRow.innerHTML += "<br>คุณทำสำเร็จ!คุณทำสำเร็จ!";
		bottomRow.innerHTML += "<br>ยินดีด้วย! คุณชนะ!<br><button onclick='restartGame()'class='buttonStyle'>เริ่มเกมใหม่!</button><br>";
		playerMoves.style.visibility = "hidden";
		battleMusic.pause();
		battleMusic.currentTime = 0;
		victoryMusic.play();
